...
public class Receiver extends Observable implements Runnable {
	...
	public Receiver(ArrayList<String> hosts, int processNumber, Clock clock, ServerSocket server) throws IOException {
		...
		this.clock = clock;
		/*
		 * permission = new ArrayList<>(hosts.size()); for (int i = 0; i <
		 * permission.size(); i++) permission.set(i, false);
		 */
	}
			...
			try {
				...
				if (rawMessage.contains("RequestMessage")) {
					...
					if (msg.getPid() != processNumber) {
						...
						/*
						Commons.log(
								"/RECEIVER: REQUESTMESSAGE from [PROCESS - " + msg.getPid() + "] at t =" + clock.peek(),
								processNumber);
								*/
					}
					...
					if (msg.getPid() != processNumber) {
						...
						clock.update(msg.getTimestamp());
						setChanged();
						notifyObservers("REPLY_FROM:" + msg.getPid());
					}
				}
			} catch (IOException e) {
				e.printStackTrace();
